                        ...
                        <div class="carousel-item <?= $active_class ?>" data-bs-interval="5000">
                            ...
                            <div class="carousel-caption hero-caption text-start">
                                <?php if (!empty($row['title'])): ?>
                                    <h1 class="display-3 fw-bold text-white mb-3 animate__animated animate__fadeInDown text-uppercase">
                                        <?= htmlspecialchars($row['title']) ?></h1>
                                <?php endif; ?>
                                <?php if (!empty($row['link'])): ?>
                                    <a href="<?= htmlspecialchars($row['link']) ?>"
                                        class="btn btn-light btn-lg rounded-pill px-5 fw-bold animate__animated animate__fadeInUp">Shop
                                        Now</a>
                                <?php endif; ?>
                            </div>
                        </div>
...
<div class="container pb-5 <?= $is_search_or_cat ? 'pt-md-5 mt-4' : '' ?>" id="collection">
    <div class="text-center mb-5 <?= $is_search_or_cat ? 'mt-md-2' : '' ?>">
        <?php if (!empty($_GET['search'])): ?>
            <h2 class="fw-bold display-5 mb-3 gradient-text border-bottom pb-3 px-2 text-wrap">Search Results for
                "<?= htmlspecialchars($_GET['search']) ?>"</h2>
        <?php else: ?>
                    ...
                    <div class="col-md-6">
                        <div class="offer-card p-4 rounded-4 d-flex justify-content-between align-items-center position-relative overflow-hidden"
                            ...
                            <div class="position-relative z-1">
                                <span
                                    class="badge bg-white text-dark mb-2 animate__animated animate__fadeIn"><?= htmlspecialchars($offer['discount_text']) ?></span>
                                <h3 class="fw-bold mb-1 animate__animated animate__fadeInUp">
                                    <?= htmlspecialchars($offer['title']) ?></h3>
                                <p class="mb-0 opacity-75 small animate__animated animate__fadeInUp">
                                    <?= htmlspecialchars($offer['description']) ?></p>
                            </div>
                            <div class="z-1 ms-3">
                                <?php if (!empty($offer['link_url'])): ?>
                                    <a href="<?= htmlspecialchars($offer['link_url']) ?>"
                                        class="btn btn-light rounded-pill fw-bold text-uppercase"
                                        style="color: <?= $bg_color ?>">Shop</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
